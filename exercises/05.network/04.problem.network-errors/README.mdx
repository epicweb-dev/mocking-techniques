...
👨‍💼 Once you can reproduce the network error in the test, implement the error handling in <InlineFile file="get-auth-token.ts" /> (handle the `fetch()` promise rejections) and add the respective assertions in the new test case.
