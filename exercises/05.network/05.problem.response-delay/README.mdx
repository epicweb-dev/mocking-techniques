...
Right now, we've accounted for the successful and error responses, and also for the network errors. However, network connectivity is tricky. What if the response _never arrives_ at all and just hangs? This can easily happen if the server is overloaded or the network is just slow.
...
:owl: I've already added the request timeout handling to <InlineFile file="get-auth-token.ts" /> so make sure to check out what's the intention there!
