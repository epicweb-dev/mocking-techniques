...
<CodeFile file="print-server-url.ts" />
...
👨‍💼 Rewrite <InlineFile file="print-server-url.test.ts" /> test suite to use the `vi.spyOn()` API from Vitest and spy on the global `console.log` method. Adjust the setup phase and the assertions to reflect the changes.
