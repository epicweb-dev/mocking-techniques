...
👨‍💼 The time has come for you to get to know `vi.stubEnv()`. In this exercise, complete new test cases for the <InlineFile file="emitter.test.ts" /> test suite by mocking the value of `process.env.NODE_ENV` using the built-in `vi.stubEnv()` API in Vitest. And remember, you will be asserting on `console.log` calls too!
