...
> Notice that I am spying on the prototype itself, which means that any `OrderController` instances created will automatically have the spy around the `.isItemInStock()` method.
